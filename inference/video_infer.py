...
Uses frame interval and spatial cache to avoid repeated OCR for the same vehicle.
"""
...
# OCR cache: avoid running OCR repeatedly for same vehicle (by position)
# Format: list of (center_x, center_y, plate_text, frame_number)
_ocr_cache = []
_OCR_CACHE_MAX_AGE = 30  # frames
_OCR_CACHE_DIST_THRESH = 80  # pixels
...
def _add_to_ocr_cache(cx, cy, plate, frame_count):
    ...
    _ocr_cache = [(x, y, p, f) for x, y, p, f in _ocr_cache if frame_count - f <= _OCR_CACHE_MAX_AGE]
...
def process_video(ip, op, conf):
    ...
    Detects violations (rider + motorcycle + no_helmet), runs plate OCR,
    draws red boxes, logs to CSV, saves evidence. Uses frame interval and OCR cache.
    """
    ...
    frame_count = 0
    process_interval = 5  # Run detection every 5 frames
    ...
    while cap.isOpened():
        ...
        if frame_count % process_interval == 1:
            ...
            for v in vehicles:
                ...
                if v["has_violation"]:
                    ...
                    # Always run plate detection for bounding box; only run OCR when not cached
                    ...
                    global_px1 = global_py1 = global_px2 = global_py2 = 0
                    if result is not None:
                    ...
                    # Log and save evidence only when we ran OCR (first time seeing this vehicle)
                    if ran_ocr:
        ...
        for item in cached_plates:
            if len(item) == 9:
                vx1, vy1, vx2, vy2, px1, py1, px2, py2, plate_num = item
